...
Mit dem \textit{feature\_extractor} werden in einem ersten Schritt mithilfe eines \textit{\gls{vfh}} oder \textit{\gls{cnn}}, Merkmale aus Bildern der Objekte extrahiert. Mit den gewonnenen Daten trainiert der \textit{classifier\_trainer} einen \gls{klassifikator}. Diese können als Annotatoren in eine Perzeptionspipeline von \robosherlock eingefügt werden und stehen in drei verschiedene Implementationen zur Verfügung: \textit{\gls{rf}}, \textit{\gls{svm}} oder \textit{\gls{knn}}.
...
In einem ersten Schritt müssen die Gewichtungen erlernt werden. Dazu werden Trainingsdaten in benötigt. Im folgenden ist ein Auszug gegeben: 
\todo{Auszug}
