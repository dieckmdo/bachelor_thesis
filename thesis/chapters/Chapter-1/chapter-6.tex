...
Es wird dementsprechend von einem ungleich verteilten Datensatz ausgegangen. Bei der Berechnung der Fehlergrößen \gls{precision}, \gls{recall} und \gls{f1score} der Klassifikation des gesamten Datensatzes, wird deshalb der \textit{gewichtete} Durchschnitt der Fehlergrößen pro Instanz/Klasse verwendet. \par
...
\begin{figure}
\centering
	\includegraphics[scale=.3]{img/chapter6/RFClassifierGTClass.png}
...
\begin{figure}
\centering
	\includegraphics[scale=.3]{img/chapter6/SVMClassifierGTClass.png}
...
\begin{figure}
\centering
	\includegraphics[scale=.3]{img/chapter6/SVMClassifierGTInstance.png}
...
Die Konfusionmatrizen sind in den Abbildungen \ref{fig:UnrealRealMixedGTClass_confMatrix} und \ref{fig:UnrealRealMixedGTInstance_confMatrix} zu finden. Die Kenngrößen für die gesamte Klassifikation findet sich in Tabelle \ref{tab:classification_all} in den letzten beiden Zeilen. Für beide \gls{gt}s hat sich die Performance gesteigert, sodass die beide bei allen Werten im Bereich um die 90\% liegen. Die meisten großen Fehlerquellen wurden durch das gemischte Trainingsset eliminiert. Nur die Messer und Gabeln bleiben deutlich hinter dem Rest zurück. Allerdings haben sich die Werte für die Messer deutlich verbessert und liegen nun im Bereich der Gabeln und nicht wie zuvor deutlich drunter.   
